	...
	private function renderToolbox( $record ) {
		...
		// editor-action already performed; display "undo" + details
			...
			if ( $last ) {
				...
				// hide & request (monitors), oversight & decline (oversighters)
				if ( $this->isAllowed( 'aft-monitor' ) && !$this->isAllowed( 'aft-oversighter' ) ) { // monitors only
					if ( !$record->isHidden() ) {
				...
				} elseif ( $this->isAllowed( 'aft-oversighter' ) ) { // oversighters only
					if ( !$record->isOversighted() ) {
