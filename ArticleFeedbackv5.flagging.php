...
class ArticleFeedbackv5Flagging {
	...
	public function run( $flag, $notes = '', $toggle = false, $source = 'unknown' ) {
		...
		if (
			...
			!ArticleFeedbackv5Model::canPerformAction( $flag, $this->user ) &&
			// users are always allowed to flag their own feedback
			!( $this->user->getId() && $this->user->getId() == intval( $this->feedback->user ) )
		) {
	...
	private function oversight( $notes, $toggle ) {
		...
		$this->feedback->oversight = 1;
		ArticleFeedbackv5Log::logActivity( __FUNCTION__, $this->feedback->page, $this->feedback->id, $notes, $this->user, array( 'source' => $this->source ) );
		...
		if ( !$this->feedback->isHidden() ) {
			 ...
			 */
			$this->feedback->hide = 1;
			$this->feedback->autohide = 1;
			ArticleFeedbackv5Log::logActivity( 'autohide', $this->feedback->page, $this->feedback->id, $notes, null, array( 'source' => $this->source ) );
	...
	private function unoversight( $notes, $toggle ) {
		...
		$this->feedback->oversight = 0;
		$this->feedback->request = 0;
		ArticleFeedbackv5Log::logActivity( __FUNCTION__, $this->feedback->page, $this->feedback->id, $notes, $this->user, array( 'source' => $this->source ) );

		// un-hide if autohidden
		if ( $this->feedback->hide && $this->feedback->autohide ) {
			$this->feedback->autohide = 0;
			$this->feedback->hide = 0;
			ArticleFeedbackv5Log::logActivity( 'unhide', $this->feedback->page, $this->feedback->id, $notes, null, array( 'source' => $this->source ) );
		}
	...
	private function unhide( $notes, $toggle ) {
		...
		if ( $this->feedback->flag && $this->feedback->autoflag ) {
	...
	private function flag( $notes, $toggle ) {
		...
		$this->feedback->{$flag}++;
		ArticleFeedbackv5Log::logActivity( $flag, $this->feedback->page, $this->feedback->id, $notes, $this->isSystemCall() ? null : $this->user, array( 'source' => $this->source ) );
		...
		$this->results['abuse_count'] = $this->feedback->flag + $this->feedback->autoflag;
	...
	private function unflag( $notes, $toggle ) {
		if ( $this->feedback->flag <= 0 ) {
			$this->feedback->autoflag = 0;
		} else {
			$this->feedback->flag--;
		}
		ArticleFeedbackv5Log::logActivity( __FUNCTION__, $this->feedback->page, $this->feedback->id, $notes, $this->user, array( 'source' => $this->source ) );
	...
	private function clear_flags( $notes, $toggle ) {
		$this->feedback->autoflag = 0;
		$this->feedback->flag = 0;
		ArticleFeedbackv5Log::logActivity( 'clear-flags', $this->feedback->page, $this->feedback->id, $notes, null, array( 'source' => $this->source ) );
	...
	private function undo_helpful( $notes, $toggle ) {
		$this->feedback->helpful--;
		ArticleFeedbackv5Log::logActivity( 'undo-helpful', $this->feedback->page, $this->feedback->id, $notes, $this->user, array( 'source' => $this->source ) );
	...
	private function undo_unhelpful( $notes, $toggle ) {
		$this->feedback->unhelpful--;
		ArticleFeedbackv5Log::logActivity( 'undo-unhelpful', $this->feedback->page, $this->feedback->id, $notes, $this->user, array( 'source' => $this->source ) );
