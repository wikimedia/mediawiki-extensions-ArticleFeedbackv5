...
class ArticleFeedbackv5Model extends DataModel {
	...
	// @todo: phpdoc etc, but first I have to complete this method and where it's called,
	// but still trying to figure out how to tackle the "update-list" problem
	function matchConditions( $list ) {
		$match = true;

		foreach ( self::$lists[$list]['conds'] as $condition ) {
			$match &= ( eval( $condition ) ); // oh how I hate doing this..
		}
	 ...
	 * @param int $page The article page id to fetch data for (0 for central feedback page)
	 * @param int $offset The offset to start from (usually a multiple of 12, the limit)
	 * @return array
	 */
	public static function getList( $name, $page, $offset = 0 ) {
		global $wgUser;
		...
		return parent::getList( $name, $page, $offset );
	}
