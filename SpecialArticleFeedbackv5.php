		...
		$count =
			Html::rawElement(
				...
				$this->msg( 'articlefeedbackv5-special-count-total',
					Html::element( 'span', array(), $totalCount ),
					$totalCount
				)->text() .
				$watchlistLink
			);
...
//		if ( $this->pageId ) { // @todo: not for central feedback page?
			$foundTotalCount = $totalCount - ArticleFeedbackv5Model::getCount( 'inappropriate', $this->pageId );
			$found = ArticleFeedbackv5Model::getCountFound( $this->pageId ) / ( $foundTotalCount ?: 1 ) * 100;
