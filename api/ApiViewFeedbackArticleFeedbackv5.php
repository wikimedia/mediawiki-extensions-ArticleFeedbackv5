...
class ApiViewFeedbackArticleFeedbackv5 extends ApiQueryBase {
	...
	public function execute() {
		...
		$result->addValue( $this->getModuleName(), 'more', $records->hasMore() );
		$result->addValue( $this->getModuleName(), 'offset', $records->nextOffset() );
		...
		if ( $params['feedbackid'] ) {
			...
			if ( $record ) {
				return new DataModelList(
					array( array( 'id' => $record->id, 'shard' => $record->page ) ),
