...
class ApiViewActivityArticleFeedbackv5 extends ApiQueryBase {
	...
	public function execute() {
		...
		if ( !$wgUser->isAllowed( 'aft-editor' ) ) {
			$this->dieWithError(
				new RawMessage( "You don't have permission to view feedback activity" ),
				'permissiondenied'
			);
		}
		...
		if ( !$pageObj->exists() ) {
			$this->dieWithError( 'notanarticle', 'notanarticle' );
		}
		...
		if ( !$feedback ) {
			$this->dieWithError(
				new RawMessage( 'Feedback does not exist' ),
				'invalidfeedbackid'
			);
		}
		...
		if ( !$page ) {
			$this->dieWithError(
				new RawMessage( "Page for feedback does not exist" ),
				'invalidfeedbackid'
			);
		}
		...
		try {
			...
			$this->dieWithError( new RawMessage( $e->getMessage() ), $e->getCode() );
		}
