...
class ApiArticleFeedbackv5 extends ApiBase {
	...
	public function execute() {
		...
		if ( !ApiArticleFeedbackv5Utils::isFeedbackEnabled( $params['pageid'] ) ) {
			$this->dieUsage(
		...
		$feedback->page = $params['pageid'];
		$feedback->page_revision = $params['revid'];
		$feedback->user = $user->getId();
		$feedback->user_text = $user->getName();
		$feedback->user_token = $params['anontoken'];
		$feedback->form = $params['bucket'];
		$feedback->cta = $params['cta'];
		$feedback->link = $params['link'];
		$feedback->rating = $params['found'];
		...
		if ( $wgArticleFeedbackv5AbuseFiltering ) {
			if ( ApiArticleFeedbackv5Utils::validateSpamRegex( $feedback->comment ) ) {
				$this->dieUsage( "Comment was flagged as abusive by SpamRegex", 'articlefeedbackv5-error-abuse' );
			} elseif ( ApiArticleFeedbackv5Utils::validateSpamBlacklist( $feedback->comment, $feedback->page ) ) {
		...
		$flagger = new ArticleFeedbackv5Flagging( null, $feedback->id, $feedback->page );
		...
		$permalinkTitle = Title::newFromText( "ArticleFeedbackv5/$page/$feedback->id", NS_SPECIAL );
