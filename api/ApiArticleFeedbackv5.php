...
//			$this->dieUsage( $e->getMessage(), 'inserterror' ); // easier when debugging: show exact exception message
		...
		// create notification via Echo extension (if feedback has textual comment)
		if ( $feedback->aft_has_comment && class_exists( 'EchoNotifier' ) ) {
			EchoEvent::create( array(
				...
				'extra' => array(
					...
					'aft_comment' => $feedback->aft_comment,
				),
				'agent' => $this->getUser(),
			) );
		}
		...
		$this->getResult()->addValue(
			...
			array(
				...
				'aft_url'     => $special->getLinkUrl( array( 'ref' => 'cta' ) ),
				'permalink'   => $permalink->getLinkUrl( array( 'ref' => 'cta' ) ),
			)
		);
