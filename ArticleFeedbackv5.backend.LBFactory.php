...
class ArticleFeedbackv5BackendLBFactory extends DataModelBackendLBFactory {
	...
	public function get( $id, $shard ) {
		...
		$conds = array();
		if ( $id ) {
			$conds["$this->table.$this->idColumn"] = $id;
		}
		if ( $shard ) {
			$conds["$this->table.$this->shardColumn"] = $shard;
		}

		return $this->getDB( DB_SLAVE )->select(
			array( $this->table, 'aft_feedback_blob' ),
			array( '*', "IFNULL(aft_feedback_blob.comment, $this->table.comment) AS comment", "$this->table.id AS id" ),
			$conds,
			__METHOD__,
			array(),
			array(
				'aft_feedback_blob' => array(
					'LEFT JOIN',
					array(
						"aft_feedback_blob.id = $this->table.id"
					)
				)
			)
		);
