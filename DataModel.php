...
*/
	...
	public static function get( $id, $shard ) {
		...
		if ( $entry === false ) {
			...
			$row = array_shift( static::getFromDB( $partition, $conds ) );
	...
	public static function getList( $name, $shard = null, $offset = 0, $sort = 'ASC' ) {
		...
		foreach ( $missing as $shard => $ids ) {
			...
			$rows = static::getFromDB( $partition, $conds );
		 ...
		 * requested, load & return those entries
		 */
	 ...
	 *
	 * @param string $name The list name (see static::$lists)
	 * @param mixed $shard The shard name
	...
	protected function updateCountCache( $name, $shard, $difference ) {
		...
		while ( true ) {
			...
			$success = $wgMemc->cas( $cas, $key, $count );
		...
		foreach ( array( $shard, null ) as $shard ) {
			...
			for ( $i = 0; $i <= $count; $i = $i + static::LIST_LIMIT ) {
				...
				foreach ( array( 'ASC', 'DESC' ) as $sort ) {
					$key = wfMemcKey( get_called_class(), 'getList', $name, $shard, $sort );
					$wgMemc->delete( $key );
				}
			}
		}
	...
	protected static function getListFromDB( $name, $shard, $offset, $limit, $sort ) {
		...
		$storeGroup = RDBStoreGroup::singleton();
		$store = $storeGroup->getForTable( 'lists' );
		$partition = $store->getPartition( 'lists', 'list_name', $name );
	...
	protected function generateId() {
		...
		while ( true ) {
			$maxId = $wgMemc->get( $key, $cas );

			// failsafe: fetch largest id from DB if it's not in cache
			if ( $maxId === false ) {
				$maxId = static::getIdFromDB();
			}
	 ...
	 * @return int
	 */
	protected function getIdFromDB() {
		$maxId = 0;

		$storeGroup = RDBStoreGroup::singleton();
		$store = $storeGroup->getForTable( static::getTable() );

		foreach ( $store->getAllPartitions( static::getTable(), static::getIdColumn() ) as $partition ) {
