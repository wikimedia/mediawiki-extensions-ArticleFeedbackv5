...
*/
	...
	protected function updateCountCache( $name, $shard, $difference ) {
		...
		foreach ( array( $shard, null ) as $shard ) {
			 ...
			 * @use mixed $shard The shard name
			 * @use int $different The different to apply to current count
			 * @return int
			 */
			$callback = function( BagOStuff $cache, $key, $existingValue ) use ( $name, $shard, $difference ) {
				// if cache is stale, get it from DB
				if ( $existingValue === false ) {
					return static::getCountFromDB( $name, $shard );
				}
			...
			for ( $i = 0; $i < 15; $i++ ) {
				// @todo: cas/merge is not yet implemented in current BagOStuff classes, though see https://gerrit.wikimedia.org/r/#/c/25879/
	...
	protected static function generateId() {
		 ...
		 * @return int
		 */
		$callback = function( BagOStuff $cache, $key, $existingValue ) use ( &$newId ) {
