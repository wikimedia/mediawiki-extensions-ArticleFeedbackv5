...
*/
	...
	public function insert() {
		 ...
		 */
		$list = static::getList( 'all', 0, 'DESC' ); // @todo: order by timestamp desc // @todo: assumes list 'all' exists, keep an internal list solely for this purpose?
		if ( count( $list ) > 0 ) {
			$last = array_shift( $list );
			$this->{static::getIdColumn()} = $last->{static::getIdColumn()} + 1;
		} else {
			$this->{static::getIdColumn()} = 1;
		}
	...
	public function updateLists() { // @todo: change to protected once fully tested
		$current = static::loadFromId( $this->{static::getIdColumn()}, $this->{static::getShardColumn()} );
		// @todo: use this to calculate current ;)

		$lists = array();
		foreach ( static::$lists as $list => $properties ) {
			$lists[$list] = static::matchListConditions( $list );
		}
...
exit;
		...
		foreach ( $lists as $list => $match ) {
			if ( $match ) {
				...
				if ( /* @todo: && doesn't exist in old list */ ) {
			...
			} elseif ( /* @todo && exists in old list */ ) {
				// @todo
			}
		}
